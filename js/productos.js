    ...
    .then(response => {
      ...
      for (let i in response) {
        output += `<tr>
      			...
      			<td class="mt-0 mb-0"><button class="btn btn-primary mt-0 mb-0" " onclick = "listarFactura(${i});calcular();"  >Agregar</button></td>
      		</tr>`;

      }
      document.getElementById('tabla-productos').innerHTML = output;
    }).catch(error => console.log(error));
  //este fetch me trae el id de la ultima insercion de factura
    ...
    .then(respuesta => {
      ...
      for (let i in respuesta) {
        salida += `<tr>
        			...
        			<td class="pt-2 pb-2"><button class="btn btn-primary mt-0 mb-0" onclick = "listarCliente(${i});"  >Agregar</button></td>
        		</tr>`;
      }
...
function listarFactura(indice) {
  ...
  document.getElementById('cuerpo-tabla').innerHTML +=
    `<tr class="align-top  " >
				 ...
				 <td class="font-weight-normal" >${contador}</td>
				 <td  class="pt-3 pb-0 pl-4">  <a type="button" class="borrar" value="Eliminar" onclick="limpiarFactura(${indice});">
...
function calcular() {
  ...
  filas.forEach(function(e) {
    ...
    precio = total;
    impuesto = total*0.18;
    impuestofix = impuesto.toFixed(2);
    precio_final = precio*0.18 + precio;
    precio_final_fix = precio_final.toFixed(2);
  });
  document.getElementById('div-precio').innerHTML = precio;
			 
  document.getElementById('total-factura').innerHTML = `
		 			<input id="precio-neto" class="form-control" type="text" placeholder="Lps ${precio_final}" readonly value= "${precio_final_fix} " name="total1" >
		 			 `
  document.getElementById('impuesto').innerHTML = `
				 			<input id="impuesto-factura" class="form-control" type="text" placeholder="Lps ${precio*0.18}" readonly value="${impuestofix} " name="impuesto1" >
				 			 `
}
...
function listarCliente(indice) {
  ...
  document.getElementById('info-cliente').innerHTML =
    `<h4 class="card-title">${global_clientes.NOMBRE_PERSONA} ${global_clientes.APELLIDO_PERSONA} </h4>
		...
		<button class="btn btn-primary borrar  " type="button" onclick="limpiarCliente();">Eliminar  <i class="fas fa-trash"></i></button>
